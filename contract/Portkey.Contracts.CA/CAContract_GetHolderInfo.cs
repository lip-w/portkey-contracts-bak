...
public partial class CAContract
{
    public override GetHolderInfoOutput GetHolderInfo(GetHolderInfoInput input)
    {
        ...
        Assert(!(input!.CaHash == null && string.IsNullOrEmpty(input.LoginGuardianAccount)),
            $"CaHash is null, and loginGuardianAccount is empty: {input.CaHash}, {input.LoginGuardianAccount}");
        ...
        else
        {
            ...
            var caHash = State.LoginGuardianAccountMap[input.LoginGuardianAccount][input.VerifierId];
            Assert(caHash != null,
                $"Not found ca_hash by a the loginGuardianAccount {input.LoginGuardianAccount} with VerifierId {input.VerifierId}.");
